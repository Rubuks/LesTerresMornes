...
const OBJETS={
hirondelle:{img:'v_heal',n:'w.item.hirondelle.name',em:"🧪",cls:"loup",p:45,tox:25,d:'w.item.hirondelle.desc',use(){S.pv=Math.min(S.pvMax,S.pv+30);C.hst.regen=6;C.hst.regenN=2;log(t('w.itemlog.hirondelle.drink'));}},
Fureur:{img:'v_vigor',n:'w.item.Fureur.name',em:"⚗️",cls:"loup",p:65,tox:25,d:'w.item.Fureur.desc',use(){C.hst.Fureur=3;log(t('w.itemlog.Fureur.drink'));}},
...
Cendre-Aveugle:{img:'v_bomb',n:'w.item.Cendre-Aveugle.name',em:"💨",cls:"loup",p:60,tox:10,sfx:"crit",d:'w.item.Cendre-Aveugle.desc',use(){const e=curFoe();if(!e)return;e.st.etourdi=1;if(e.vole){e.chute+=2;log(t('w.itemlog.Cendre-Aveugle.fly'));}else log(t('w.itemlog.Cendre-Aveugle.ground'));}},
puffball:{img:'v_toxin',n:'w.item.puffball.name',em:"🍄",cls:"loup",p:50,tox:10,sfx:"signe",d:'w.item.puffball.desc',use(){C.foes.forEach(e=>{if(e.vivant)e.st.poison={d:5,n:3};});log(t('w.itemlog.puffball.cloud'));}},
...
nectar:{img:'v_heal',n:'w.item.nectar.name',em:"🌙",cls:"sorciere",p:50,tox:15,d:'w.item.nectar.desc',use(){S.pv=Math.min(S.pvMax,S.pv+35);C.hst.regen=6;C.hst.regenN=2;log(t('w.itemlog.nectar.drink'));}},
poudre:{img:'v_vigor',n:'w.item.poudre.name',em:"💠",cls:"sorciere",p:65,tox:15,d:'w.item.poudre.desc',use(){C.hst.Fureur=3;log(t('w.itemlog.poudre.drink'));}},
voile:{img:'v_mist',n:'w.item.voile.name',em:"🌫️",cls:"sorciere",p:55,tox:15,d:'w.item.voile.desc',use(){C.hst.chat=3;log(t('w.itemlog.voile.drink'));}},
cristal:{img:'v_crystal',n:'w.item.cristal.name',em:"⚡",cls:"sorciere",p:60,sfx:"crit",d:'w.item.cristal.desc',use(){blesserEnnemi(14,null,'⚡ '+t('w.item.cristal.name'),false,false);if(!C||C.fini)return;const e=curFoe();if(e&&e.vivant&&e.vole){e.chute++;log(t('w.itemlog.cristal.fly'));}if(e&&e.vivant&&e.pvE>0&&Math.random()<.3){e.st.etourdi=1;log(t('w.itemlog.cristal.stun'));}}},
glyphe:{img:'v_glyph',n:'w.item.glyphe.name',em:"🔯",cls:"sorciere",p:75,d:'w.item.glyphe.desc',use(){C.meteore=false;S.pv=Math.min(S.pvMax,S.pv+10);log(t('w.itemlog.glyphe.renew'));}},
esprit:{img:'v_mug',n:'w.item.esprit.name',em:"🍺",cls:"mercenaire",p:45,d:'w.item.esprit.desc',use(){S.pv=Math.min(S.pvMax,S.pv+25);C.hst.Fureur=Math.max(C.hst.Fureur,2);log(t('w.itemlog.esprit.drink'));}},
...
gnome:{img:'v_mug',n:'w.item.gnome.name',em:"🥃",cls:"mercenaire",p:65,d:'w.item.gnome.desc',use(){C.hst.Fureur=3;log(t('w.itemlog.gnome.roar'));}},
runeFureur:{img:'v_rune',n:'w.item.runeFureur.name',em:"🪨",cls:"mercenaire",p:70,d:'w.item.runeFureur.desc',use(){C.rune=true;C.foes.forEach(e=>{if(e.vivant&&e.st.intangible>0){e.st.intangible=0;log(t('w.log.intangibleBreak',{n:t(e.nom)}));}});log(t('w.itemlog.runeFureur.etch'));}},
...
mixture:{img:'v_herb',n:'w.item.mixture.name',em:"🌿",cls:"eclaireuse",p:45,tox:15,d:'w.item.mixture.desc',use(){S.pv=Math.min(S.pvMax,S.pv+30);C.hst.regen=6;C.hst.regenN=2;log(t('w.itemlog.mixture.drink'));}},
seve:{img:'v_mist',n:'w.item.seve.name',em:"🌙",cls:"eclaireuse",p:55,tox:15,d:'w.item.seve.desc',use(){C.hst.chat=3;log(t('w.itemlog.seve.drink'));}},
elan:{img:'v_vigor',n:'w.item.elan.name',em:"🍃",cls:"eclaireuse",p:60,tox:15,d:'w.item.elan.desc',use(){C.hst.Fureur=3;log(t('w.itemlog.elan.drink'));}},
bague:{img:'v_rune',n:'w.item.ring.name',em:"💍",kind:'quest',d:'w.item.ring.desc'}
};
function soinClasse(){return{loup:"hirondelle",sorciere:"nectar",mercenaire:"esprit",eclaireuse:"mixture"}[S.cls];}
